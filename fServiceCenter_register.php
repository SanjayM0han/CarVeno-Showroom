...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($emailExists) {
        ...
        $usernameExistsUrl = $databaseUrl . 'sregistered/' . urlencode($username) . '.json';
        ...
        if ($usernameExistsResponse === 'null') {
            // Construct the URL for storing user data under "servicecenters" node
            $registeredUrl = $databaseUrl . 'servicecenters/' . urlencode($username) . '.json';

            // Set user data to be stored
            $userData = json_encode(['email' => $email]); // You may include other user data as needed
            ...
            header("Location: ServiceCenter_login.php");
            exit();
        } else {
...
<body class="register-body">
    <div class="container">
        <form id="registration_form" action="fServiceCenter_register.php" method="post" class="form">
            <h2>Registration</h2>
            <?php if(isset($error)) { ?>
            ...
            <p class="login-link">Already have an account? <a href="ServiceCenter_login.php">Sign In</a></p>
        </form>
    </div>
</body>
</html>
